...
- decisive_axes: the array of section keys that carry the decision (the ones NOT tied) — e.g. ["monetization","competition"]. MANDATORY FLOOR — a score-moving axis can NEVER be invisible here: market_demand, monetization, and originality are the ONLY three axes in the overall score, so if any one of them has two scores differing by 1.0 or more, that axis MUST appear in decisive_axes — it carried the scored gap the founder sees, by arithmetic, no matter how the section's prose framed it. This holds even when the founder/effort axes (technical_complexity, risks, execution_reality) are the louder narrative separation: those three never enter the overall score, so they may JOIN decisive_axes but can never DISPLACE a score-moving merit axis. Never drop a merit axis that moved the number because a softer axis feels more decisive — the axis that determined the score the founder is looking at always carries the decision.
